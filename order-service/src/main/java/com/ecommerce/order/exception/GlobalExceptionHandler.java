...
public class GlobalExceptionHandler {
    ...
    /**
     * 处理业务层 RuntimeException（如「订单不存在」「库存不足」），把真实原因透传给调用方
     */
    ...
    public ResponseEntity<Map<String, Object>> handleRuntimeException(
        ...
        log.warn("order-service业务异常: path={}, msg={}", request.getRequestURI(), msg);
    ...
    public ResponseEntity<Map<String, Object>> handleException(
            Exception exception,
            HttpServletRequest request) {
        log.error("order-service未处理异常: path={}", request.getRequestURI(), exception);
